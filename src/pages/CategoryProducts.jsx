...
const CategoryProducts = () => {
    ...
    // Decode the subcategory from URL
    ...
    // Mock products based on subcategory
    const mockProducts = Array.from({ length: 12 }, (_, i) => {
        const base = data?.products?.length ? data.products[i % data.products.length] : undefined
        const image = base?.image || base?.categoryImage || 'https://images.unsplash.com/photo-1578749556568-bc2c40e68b61?w=300&q=80'
        return {
            id: i + 1,
            partNumber: `${i + 1000}920`,
            name: `${decodedSubcategory} ${i + 1}`,
            price: 225 + (i * 15),
            image,
            replaces: `Replaces Part(s) 30${i}945`
        }
    })
            ...
            <div className="container-fluid py-4">
                ...
                <nav aria-label="breadcrumb" className="mb-4">
                    <ol className="breadcrumb">
                        ...
                        <li className="breadcrumb-item active" aria-current="page">{decodedSubcategory}</li>
                    </ol>
                </nav>
                        ...
                        <div className="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                <h4 className="fw-bold mb-0">
                                    {mockProducts.length} Results for <span className="text-dark">{decodedSubcategory}</span>
                                </h4>
                            </div>
                            ...
                            <div className="row g-3">
                                {mockProducts.map(product => (
                                    <div key={product.id} className="col-lg-6 col-xl-4">
                                        <button
                                            onClick={() => navigate(`/product/${category}/${encodeURIComponent(product.name)}`)}
                                        ...
                                        >
                                            <div className="card border-0 shadow-sm h-100">
                                                ...
                                                <div className="card-body d-flex flex-column">
                                                    <p className="text-muted small mb-1">Part #{product.partNumber}</p>
                                                    <h6 className="card-title fw-semibold mb-2 text-dark">{product.name}</h6>
                                                    <p className="text-muted small mb-2">{product.replaces}</p>
                                                    <h5 className="text-primary fw-bold mb-3">${product.price.toFixed(2)}</h5>
                                                    <button 
                                                        className="btn btn-outline-primary mt-auto"
                                                        onClick={(e) => {
                                                            e.stopPropagation()
                                                            const productForCart = {
                                                                id: `${category}:${decodedSubcategory}:${product.id}`,
                                                                title: product.name,
                                                                price: product.price,
                                                                image: product.image
                                                            }
                                                ...
                                                <div className="col-md-9">
                                                    <div className="card-body d-flex justify-content-between align-items-start">
                                                        <div className="flex-grow-1">
                                                            <p className="text-muted small mb-1">Part #{product.partNumber}</p>
                                                            <h6 className="card-title fw-semibold mb-2 text-dark">{product.name}</h6>
                                                            <p className="text-muted small mb-0">{product.replaces}</p>
                                                        </div>
